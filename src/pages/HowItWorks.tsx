...
export default function HowItWorks() {
  return <>
    ...
    <Section id="payment-methods" tone="subtle"><SectionHeading eyebrow="Payment methods" title="Meet families where they pay" description="Payment methods are surfaced through the configured SSLCOMMERZ merchant setup. Keep the live method list editable as gateway configuration evolves." />
      <Grid container spacing={{ xs: 4, md: 7 }} alignItems="center"><Grid size={{ xs: 12, md: 7 }}><Grid container spacing={2}>{PAYMENT_METHODS.map((m, i) => <Grid size={{ xs: 12, sm: 6 }} key={m.name}><Reveal delay={i * 60} sx={{ height: '100%' }}><Box sx={{ height: '100%', transition: 'transform 250ms var(--ease)', '&:hover': { transform: 'translateY(-4px)' } }}><PaymentCard name={m.name} category={m.category} icon={m.icon} /></Box></Reveal></Grid>)}</Grid><Stack direction="row" spacing={1.5} alignItems="flex-start" sx={{ mt: 3, p: 2.25, borderRadius: `${radius.lg}px`, bgcolor: color.warning[50], border: '1px solid #FDE68A' }}><Info size={16} color={color.warning[700]} /><Typography variant="caption" sx={{ color: color.warning[700] }}>The live method list follows your institution's SSLCOMMERZ merchant configuration. Confirm it before launch and edit <Box component="code" sx={{ fontFamily: 'var(--font-mono)' }}>PAYMENT_METHODS</Box> in <Box component="code" sx={{ fontFamily: 'var(--font-mono)' }}>src/content/platform.ts</Box>.</Typography></Stack></Grid><Grid size={{ xs: 12, md: 5 }}><Reveal><Box sx={{ display: 'flex', justifyContent: 'center' }}><PhoneMockup width={280} /></Box></Reveal></Grid></Grid>
    </Section>
    ...
    <Section id="technology" tone="dark"><SectionHeading eyebrow="Technology & security" title="Built for payment confidence." description="Keep security and operational visibility understandable for finance, administration and technology teams." onDark />
      ...
      <Reveal><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1.25fr .75fr' }, gap: 2.5 }}><Box sx={{ p: { xs: 2.5, md: 3.5 }, borderRadius: `${radius.xl}px`, bgcolor: 'rgba(255,255,255,.045)', border: '1px solid rgba(255,255,255,.11)' }}><Typography variant="h5" sx={{ color: '#fff', mb: 2 }}>Technology stack</Typography><Stack spacing={1}>{TECH_STACK.map((row) => <Stack key={row.layer} direction="row" justifyContent="space-between" spacing={2} sx={{ py: 1.25, borderBottom: '1px solid rgba(255,255,255,.08)' }}><Typography variant="body2" sx={{ color: '#fff', fontWeight: 600 }}>{row.layer}</Typography><Typography variant="body2" sx={{ color: 'rgba(255,255,255,.58)', textAlign: 'right' }}>{row.value}</Typography></Stack>)}</Stack></Box><Box sx={{ p: { xs: 2.5, md: 3.5 }, borderRadius: `${radius.xl}px`, bgcolor: 'linear-gradient(145deg, rgba(99,102,241,.18), rgba(255,255,255,.04))', border: '1px solid rgba(255,255,255,.11)' }}><ShieldCheck size={22} color="#A9B2FF" /><Typography variant="h5" sx={{ color: '#fff', mt: 2, mb: 1.5 }}>Security, without overclaiming</Typography><Typography variant="body2" sx={{ color: 'rgba(255,255,255,.62)', lineHeight: 1.7 }}>Payment processing is handled through the configured gateway. Platform-level certifications or compliance claims should only be added once verified with the relevant team.</Typography></Box></Box></Reveal>
    </Section>
