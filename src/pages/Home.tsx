...
const quickBenefits = [
  ...
  { icon: LockKeyhole, title: 'Advanced security', text: 'Use a trusted payment gateway while keeping the product experience focused.' },
];

export default function Home() {
  return (
    <>
      {/* Master homepage foundation + refined hero */}
      <HomeHero />

      {/* Retained from master: strong value and trust hierarchy */}
      <ValueBar />
      <TrustBar />

      {/* Added from the refined homepage: fast-scanning product benefits */}
          ...
          {quickBenefits.map(({ icon: Icon, title, text }, i) => (
            <Grid size={{ xs: 12, sm: 6, md: 3 }} key={title}>
              <Reveal delay={i * 60}>
                ...
                >
                  <Box sx={{ width: 42, height: 42, display: 'grid', placeItems: 'center', borderRadius: `${radius.md}px`, bgcolor: color.brand[50], color: color.brand[600], mb: 2 }}>
                    <Icon size={20} />
                  </Box>
      ...
      {/* Master content: platform story and audience value */}
      <PlatformStory />
      <Stakeholders />

      {/* Master process journey + refined product-first storytelling */}
      <ProcessJourney />
      <ProductShowcase />
      <Benefits />
      <UseCases />

      {/* Master security, FAQ and conversion flow */}
      <SecurityBand />
      <HomeFaq />
